...
public class Game {
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		if (getCurrentChampion() instanceof Hero) {
			...
			if(secondPlayer.getTeam().contains(getCurrentChampion()))
				for(int i = 0; i < firstPlayer.getTeam().size(); i++){
					if(firstPlayer.getTeam().get(i).compareTo(firstPlayer.getLeader()) != 0){
						if(firstPlayer.getTeam().get(i).getCurrentHP() != 0){
							targets.add(firstPlayer.getTeam().get(i));
						}
					}
				}
			else if(firstPlayer.getTeam().contains(getCurrentChampion()))
				for(int i = 0; i < secondPlayer.getTeam().size(); i++){
					if(secondPlayer.getTeam().get(i).compareTo(secondPlayer.getLeader()) != 0){
